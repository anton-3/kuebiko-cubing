...
def create_dataframe(file, timezone):
    ...
    if headers.startswith('Puzzle,Category,Time(millis),Date(millis),Scramble,Penalty,Comment'):
        ...
        return df, has_dates, timer_type, mergeable_categories
    elif "Generated By ChaoTimer" in headers:
        # ChaoTimer
...
def process_data(file, chart_by, secondary_y_axis, subx_threshold_mode, subx_override, day_end_hour, timezone,
    ...
    solves_data, has_dates, timer_type, mergeable_categories = create_dataframe(file, timezone)
    ...
    if merge_categories and mergeable_categories:
        solves_data['Category'] = 'Merged sessions'
